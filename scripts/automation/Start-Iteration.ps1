...
if (-not $LogFile) { $LogFile = Join-Path $Paths.LocalLogs 'iteration.log' }
...
function Generate-Task-Script {
    ...
    $taskFile = Join-Path $Paths.ScriptsState 'iteration-tasks.txt'
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
...
function Invoke-Full-Automation {
    ...
    $verifyScript = Join-Path $AutomationDir 'Auto-Feature-Verify.ps1'
    ...
    if (-not $SkipBuild) {
        ...
        $testScript = Join-Path $AutomationDir 'Auto-Test.ps1'
    ...
    $buildScript = Join-Path $AutomationDir 'Auto-Build.ps1'
    ...
    $apkPath = Join-Path $Paths.ApkDebug 'app-debug.apk'
    ...
    $commitScript = Join-Path $AutomationDir 'Github-push.ps1'
...
if ($success) {
    ...
    Write-Log "========================================" "SUCCESS"
    Write-Log "APK: $(Join-Path $Paths.ApkDebug 'app-debug.apk')" "SUCCESS"
