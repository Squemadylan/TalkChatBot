...
if (-not $BuildCommand) { $BuildCommand = Join-Path $AutomationDir 'Auto-Build.ps1' }
if (-not $LogFile) { $LogFile = Join-Path $Paths.LocalLogs 'auto-update.log' }
...
function Get-ChangedFilesCount {
    git -C "$RepoRoot" status --porcelain 2>$null | Measure-Object -Line | Select-Object -ExpandProperty Lines
}

function Invoke-Build {
    Write-Log "Code change detected, starting automated build..." "WARN"

    $publishScript = Join-Path $AutomationDir 'Auto-Publish.ps1'
...
Write-Log "Watch Path: $RepoRoot" "INFO"
Write-Log "========================================" "INFO"
